...
uint32_t AP_Frsky_Telem::calc_gps_latlng(bool *send_latitude)
{
    ...
    if ((*send_latitude) == true) {
        if (loc.lat < 0) {
            latlng = (uint32_t)((abs(loc.lat)/100)*6) | 0x40000000;
        } else {
            latlng = (uint32_t)((abs(loc.lat)/100)*6);
        }
        (*send_latitude) = false;
    } else {
        if (loc.lng < 0) {
            latlng = (uint32_t)((abs(loc.lng)/100)*6) | 0xC0000000;
        } else {
            latlng = (uint32_t)((abs(loc.lng)/100)*6) | 0x80000000;
        }
        (*send_latitude) = true;
    }
    return latlng;
