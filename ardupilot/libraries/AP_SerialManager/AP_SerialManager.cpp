...
const AP_Param::GroupInfo AP_SerialManager::var_info[] = {
    ...
    // @User: Standard
    AP_GROUPINFO("0_PROTOCOL",  11, AP_SerialManager, state[0].protocol, SerialProtocol_None/*SerialProtocol_MAVLink*/),
    ...
    // @User: Standard
    AP_GROUPINFO("1_BAUD", 2, AP_SerialManager, state[1].baud, 921600/1000),
...
void AP_SerialManager::init()
{
    ...
    for (uint8_t i=1; i<SERIALMANAGER_NUM_PORTS; i++) {
        ...
        if (i == 5 && state[i].protocol != SerialProtocol_None) {
            //TODO tell nsh to exit to free up this uart
