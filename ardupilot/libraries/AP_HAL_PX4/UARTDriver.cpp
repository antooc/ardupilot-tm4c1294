...
void PX4UARTDriver::begin(uint32_t b, uint16_t rxS, uint16_t txS)
{
    ...
    if (strcmp(_devpath, RT_CONSOLE_DEVICE_NAME) == 0) {
        min_tx_buffer = 4096;
        min_rx_buffer = 1024;
    }
    ...
    if (!_dev)
    {
        _dev = rt_device_find(_devpath);
        if(!_dev)
        {
            return;
        }
        err = rt_device_open(_dev, RT_DEVICE_FLAG_INT_RX);
        if(err != RT_EOK)
        {
            _dev = RT_NULL;
            return;
	...
	if (_baudrate != 0) {
	    ...
	    rt_memcpy(&config, &((struct rt_serial_device *)_dev)->config, sizeof(struct serial_configure));
...
//		tcsetattr(_fd, TCSANOW, &t);
    ...
    if (_writebuf.get_size() && _readbuf.get_size() && _dev) {
        if (!_initialised) {
        	//TODO set_usb_connected
//            if (strcmp(_devpath, "/dev/ttyACM0") == 0) {
//                ((PX4GPIO *)hal.gpio)->set_usb_connected();
//            }
            rt_kprintf("initialised %s OK %u %u\n", _devpath,
...
//    }
    ...
    ret = rt_device_write(_dev, 0, buf, n);
...
//    }
    ...
    if(rt_strcmp(_devpath, RT_CONSOLE_DEVICE_NAME) == 0)
    {
        return ret;
    }
    ret = rt_device_read(_dev, 0, buf, n);
