    ...
    , device_desc(_device_desc)
{
    ...
    snprintf(pname, sizeof(pname), "SPI:%s:%u:%u",
             device_desc.name,
             (unsigned)bus.bus, (unsigned)device_desc.device);
    perf = perf_alloc(PC_ELAPSED, pname);

    spidev = (struct rt_spi_device *)rt_device_find(device_desc.name);
    if(spidev == RT_NULL)
    {
...
//    SPI_SELECT(bus.dev, device_desc.device, false);
    rt_kprintf("SPI device %s on %u:%u at speed %u mode %u\n",
           device_desc.name,
           (unsigned)bus.bus, (unsigned)device_desc.device,
...
//    }
    ...
    if(spidev)
    {
        rt_spi_transfer(spidev, send, recv, len);
    }
    else
    {
        rt_memset(recv, 0, len);
    }
}
...
void SPIDevice::config_spidev(void)
{
...
SPIDeviceManager::get_device(const char *name)
{
    ...
    if (busp == nullptr) {
        ...
        busp->next = buses;
        busp->bus = desc.bus;
        /* rtthread的rt_spi_device自己会记录bus，不再需要应用层来记 */
