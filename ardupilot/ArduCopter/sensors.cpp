...
void Copter::log_optical_flow(void)
{
#pragma pack (1)
typedef struct
{
...
#pragma pack ()
    ...
    optflow_log_t log =
    {
         flow_x     : optflow.flowRate().x,
         flow_y     : optflow.flowRate().y,
         body_x     : optflow.bodyRate().x,
         body_y     : optflow.bodyRate().y,
         gyrx       : ahrs.get_gyro().x,
         gyry       : ahrs.get_gyro().y,
    };

    hal.uartF->logWave(&log, sizeof(log));
}
#endif  // OPTFLOW == ENABLED
